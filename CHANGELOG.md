...
- feat(Icons): Add a pluggable icon engine and a new `loco_icon` helper (`Loco::IconComponent`) that renders
  inline SVG from any installed icon library. LocoMotion bundles Heroicons (all four variants) inside the
  engine so the default `loco_icon("name")` works with zero setup, and consumers can render any other library
  (Lucide, Phosphor, Tabler, brand sets, …) by syncing it into their own app. Resolution is two-tier — the
  app's `app/assets/svg/icons` is checked first, then the bundled set — and SVGs are parsed in XML mode so
  `viewBox` and other case-sensitive attributes stay valid. The helper mirrors `hero_icon` (the `where:size-5`
  default, `css:`, `tip:`) and adds `library:` / `variant:`. Built on the framework-agnostic `icons` gem
  (keeps Rails 6.1). Added alongside the existing `hero_icon`, which is unchanged for now. Refs #204.
