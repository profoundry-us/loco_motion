...
class IconableTestComponent < LocoMotion::BaseComponent
  ...
  def call
    content = ""
    content += helpers.heroicon_tag(@left_icon, **left_icon_html) if @left_icon
    content += "Test Content"
    content += helpers.heroicon_tag(@right_icon, **right_icon_html) if @right_icon
...
RSpec.describe LocoMotion::Concerns::IconableComponent, type: :component do
  ...
  context "with custom icon HTML attributes" do
    ...
    # We don't need to test the actual rendering since that depends on the heroicon_tag helper
